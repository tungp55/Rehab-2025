...
from merge_tracks_graph import *
...
# with open('F:/MITICA/visualize GT result/graph_utils/sample_network.txt', 'r') as f:
...
nodes, edges, nodes_di, edges_di, followers_node_di, followers_edges_di = convert_nx_2_dash()
genesis_node = nodes[0]
genesis_node['classes'] = "genesis"
default_elements = nodes.copy()
default_elements.extend(edges)
...
def generate_elements(nodeData, elements, expansion_mode):
    ...
    if expansion_mode == 'followers':
        ...
        followers_nodes = followers_node_di.get(int(nodeData['id']))
        followers_edges = followers_edges_di.get(int(nodeData['id']))
