...
VIDEO_FILE = "small3.MP4"
PRED_FILE = 'GH010358_pred.txt'
GT_FILE = 'GH010358_gt.txt'
LINE_HEIGHT = 10
LEFT_BOTTOM_Y = 50
RIGHT_BOTTOM_Y = 80
LEFT_COLOR = (255, 0, 0)
...
while 1:
    ...
    elapsed_time = current_time - start_play_time
    elapsed_time_seconds = elapsed_time.total_seconds()
    current_frame = video.get(cv2.CAP_PROP_POS_FRAMES)
    # print(elapsed_time, '-', int(current_frame / frame_rate))
    if (current_frame < elapsed_time_seconds * frame_rate):
        # if hist_miss > 10 * milisecond_per_frame:
        #     seek_frame = current_frame + int(hist_miss / milisecond_per_frame)
        seek_frame = int(elapsed_time_seconds * frame_rate)
        video.set(cv2.CAP_PROP_POS_FRAMES, seek_frame)
        hist_miss = 0
    image_process_time = 0
    ...
    # text_time = time.strftime("%M:%S %Z", elapsed_time)
    ...
    cv2.putText(frame, text_time,
                ...
                fontColor,
                lineType)
    cv2.putText(frame, 'Tay trai',
                (150, 20),
                font,
                fontScale,
                LEFT_COLOR,
                lineType)
    cv2.putText(frame, 'Tay phai',
            (250, 20),
            font,
            fontScale,
            RIGHT_COLOR,
    ...
    if ret:
        ...
        break
    end_show_time = datetime.now()
    image_process_time = end_show_time - start_read_time
    # print(image_process_time)
    # print(image_process_time.total_seconds() * 1000)

    # display frame for 'playSpeed' ms, detect key input
    wait_time = int((100 / playSpeed) * 1000 / frame_rate) - \
        int(image_process_time.total_seconds() * 1000)
    if wait_time < 0:
        hist_miss += -wait_time
    key = cv2.waitKey(max(wait_time, 1))
