...
in subdirectories of txt_v1/download.
Output will be saved to merge_txt folder in each subdirectory.
"""
...
def main():
    """Main function to process all txt files in subdirectories."""
    ...
    for subdir in sorted(subdirs):
        ...
        if not txt_dir.exists():
            print(f"⚠️  No 'txt' directory found in {subdir.name}, skipping...")
            continue
        ...
        if not txt_files:
            print(f"⚠️  No txt files found in {txt_dir}, skipping...")
            continue
        
        print(f"📄 Found {len(txt_files)} txt files")
        
        # Create output directory
        output_dir = subdir / "merge_txt"
        ...
        # Summary for this subdirectory
        print(f"\n{'='*60}")
        print(f"📊 Summary for {subdir.name}:")
    ...
    print(f"   ✅ Success: {total_success}/{total_files}")
    print(f"   ❌ Errors: {total_errors}/{total_files}")
    print(f"{'='*80}\n")
